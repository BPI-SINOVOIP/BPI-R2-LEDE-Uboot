...
int rt2880_eth_initialize(bd_t *bis)
{
	...
	rx_ring = (struct PDMA_rxdesc *)(&rx_ring_cache[0]);
	tx_ring0 = (struct PDMA_txdesc *)(&tx_ring0_cache[0]);
...
{
	int i, j;
	int __attribute__ ((unused)) read_data;
	j = 0;
	while (j < 10) {
		for (i = 0; i < 32; i = i + 1) {
...
void trgmii_calibration_7623(void)
{
	...
	unsigned int __attribute__ ((unused)) init_toggle_data;
	unsigned int err_flag[5];
	unsigned int err_total_flag;
	unsigned int __attribute__ ((unused)) training_word;
	unsigned int rd_tap;

	u32  TRGMII_7623_base;
	u32  TRGMII_7623_RD_0;
	u32 __attribute__ ((unused)) TRGMII_RCK_CTRL;
...
#define DEVINFO_BASE                    0x17000000
	...
	u32 __attribute__ ((unused)) TRGMII_7530_TX_base;
...
{
	u32	i;
	u32	__attribute__ ((unused)) regValue;
	u16	wTmp;
	uchar	__attribute__ ((unused)) *temp;
...
static int do_rt3883_pseloopback(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
...
static int rt3052_switch_command(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	if (argc < 2) {
		printf("Usage:\n%s\n", cmdtp->usage);
		return 1;
...
static int rdm_ioctl(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	u32 offset, rtvalue;
	u32 address;
	u32 count = 0;
...
static int EswCntRead(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
