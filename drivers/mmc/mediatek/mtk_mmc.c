...
#else
	if (dev == 0) return &g_mtk_mmc_block_emmc;
	else return &g_mtk_mmc_block_sd;
#endif
...
static struct mmc mtk_mmc[2]; //0:emmc 1:sd
static struct mmc_data_priv mtk_mmc_data_priv[2];
...
static int mtk_msdc_initialize(bd_t *bis, int num)
{
	struct mmc *mmc = &mtk_mmc[num];

	memset(mmc, 0, sizeof(struct mmc));
	mmc->priv = &mtk_mmc_data_priv[num];
...
int board_mmc_init(bd_t *bis)
{
	 ...
	 */
	for (i = 0; i < 2; i++) {
		mtk_mmc_data_priv[i].id = i;
		status |= mtk_msdc_initialize(bis, i);
	}
